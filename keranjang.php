...
if ($action) {
    ...
    try {
        ...
        if ($action === 'update_jumlah') {
            ...
            $stmt = mysqli_prepare($conn, $sql);
            mysqli_stmt_bind_param($stmt, 'iii', $jumlah, $cart_id, $id_user);
            mysqli_stmt_execute($stmt);
            echo json_encode(['success' => true]);
            exit;
        }
...
<body>
            ...
            <?php if (!$id_user): ?>
                <div class="cart-empty text-center">
                    ...
                    <p class="cart-subtitle">
                        Untuk mengakses keranjang, Anda harus login terlebih dahulu.
                    </p>
                    <a href="login.php" class="btn cart-continue-btn">Login</a>
                </div>
            <?php else: ?>
    ...
    function renderCart(items) {
        ...
        items.forEach(item => {
            ...
            tr.innerHTML = `
                ...
                <div class="d-flex align-items-center">
                    <img src="${escapeHtml(item.image)}" class="cart-item-img me-3" alt="">
                    <div><div class="fw-semibold">${escapeHtml(item.name)}</div></div>
                </div>
                </td>
                <td class="text-center">
                <div class="qty-control d-inline-flex align-items-center justify-content-center">
                    <button type="button" class="btn btn-sm btn-qty btn-qty-minus">−</button>
                    <input type="number" class="form-control form-control-sm text-center cart-qty mx-1" value="${qty}" min="1">
                    <button type="button" class="btn btn-sm btn-qty btn-qty-plus">+</button>
                </div>
                </td>
        ...
        async function updateQty(row, newVal) {
            const cartId = row.dataset.cartId;
            await apiCall('update_jumlah', 'POST', { cart_id: cartId, jumlah: newVal });
            await loadCart();
        }
        ...
        document.querySelectorAll('.cart-qty').forEach(input => {
            input.addEventListener('change', function () {
                ...
                if (val < 1) val = 1;
                this.value = val;
                updateQty(this.closest('.cart-item'), val);
            });
        });
        ...
        document.querySelectorAll('.btn-delete-item').forEach(btn => {
            btn.addEventListener('click', async function () {
                ...
                const row = this.closest('.cart-item');
                await apiCall('delete', 'POST', { cart_id: row.dataset.cartId });
                await loadCart();
            });
        });
    }

    const isLoggedIn = <?= $id_user ? 'true' : 'false' ?>;
